...
ModbusClientSW::ModbusClientSW(): Node("ModbusClientSW"), m_mutex(new boost::mutex()){
    ...
    while(!shouldStop){
        ...
        case CHECK:{
            ...
            if(errorElectronic == ERROR){
                boost::this_thread::sleep(boost::posix_time::microseconds(config.errorTimeout));
        ...
        case DELAY:{
            boost::this_thread::sleep(boost::posix_time::milliseconds(config.minimalDelay));
...
bool ModbusClientSW::writeRegister(writeData _data){
    ...
    while(!success && counter<config.tryCounter && errorElectronic == NO_ERROR){
        boost::this_thread::sleep(boost::posix_time::milliseconds(config.retryDelay));
        success = modbus->ModbusPresetSingleRegister(_data.id.slave_address,_data.id.function_address, _data.data);
        counter++;
    }
